...
impl Fish {
    pub fn new(path: &PathBuf, name: &str, tank: &Tank) -> Self {
        ...
        let size = Size {
            height: fish_anim[0].len(),
            width: fish_anim[0][0].len(),
        };
        ...
        if fish_anim.len() != flip_anim.len() {
            error(
                &format!("fish {} has a mismatch in fish and flip length", name),
                1,
            );
        }
        if fish_anim[0].len() != flip_anim[0].len()
            || fish_anim[0][0].len() != flip_anim[0][0].len()
        {
            error(
                &format!("fish {} has a mismatch in fish and flip size", name),
                1,
            );
        }
...
impl HasColorGlyph for Fish {
    fn get_glyph(&self, row_idx: usize, glyph_idx: usize) -> Option<ColorGlyph> {
        let mut animation: &Animation = &self.fish_anim;
        if self.flip {
